    ...
    <main>
      ...
      <section className="mx-auto max-w-7xl px-5 pb-20 sm:px-8">
        <div className="grid gap-5 lg:grid-cols-3">
          {features.map((feature) => (
            <SurfaceCard key={feature.title}>
              <p className="font-mono text-sm text-indigo-300">FEATURE</p>
        ...
        <SurfaceCard className="mt-6 flex flex-col gap-6 p-8 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="text-2xl font-semibold text-white">期末综合模拟卷</h2>
            <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-400">
              当前为静态占位状态。真实功能上线后，这里会显示考试时长、题量、章节覆盖范围和开始考试按钮。
            </p>
          </div>
          <button className="rounded-lg bg-indigo-300 px-5 py-3 text-sm font-semibold text-slate-950 opacity-70">
            即将开放
          </button>
        </SurfaceCard>
      </section>
